...
const ProgressTracker = forwardRef<ProgressTrackerRef, ProgressTrackerProps>(({
  ...
  // Filter steps based on totalSteps prop
  ...
  // ❌ DISABLED: Steps are view-only, not clickable
  // const handleStepClick = (stepNumber: number) => {
  //   // Step clicking is disabled - progress tracker is view-only
  // };
    ...
    <div className={`flex flex-col items-center w-full justify-start ${className}`} style={{ overflow: 'visible', padding: '8px 0' }}>
      ...
      <div className="flex flex-row mt-1 items-center justify-center w-full px-2" style={{ overflow: 'visible', gap: '8px' }}>
        {steps.map((step, index) => (
          ...
          >
            {/* Step Number - View Only (not clickable) */}
            <div
              className={`
                ...
                ${
                  ...
                  : step.number < activeStep
                    ? 'w-6 h-6 sm:w-7 sm:h-7 bg-[#004CE9] text-white shadow-sm'
                    : 'w-6 h-6 sm:w-7 sm:h-7 bg-gray-200 text-gray-500'
                }
              `}
              style={{
